...
SPEC_TASKS_FILE="$PROJECT_ROOT/specs/001-framework-enhancements/tasks.md"
...
verify_build() {
  ...
  for f in scripts/ralph.sh scripts/sync-agent.sh scripts/sync-helpers.sh scripts/monitor.sh; do
    if [[ -f "$f" ]]; then

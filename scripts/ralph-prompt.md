...
- **Never background ANY command.** Do not use trailing `&`, `nohup`, `disown`, or "run in background" tool invocations. Run every command synchronously in the foreground and wait for it to fully return before continuing. NEVER end your turn while a process is still running or in progress. Emit the completion sentinel ONLY after all commands have returned and work is committed. A turn ending with "waiting for X to complete" and no sentinel counts as a stall and wastes an attempt. Do not run non-terminating commands like `tail -f`, `watch`, `npm run dev`, or `python -m http.server` — they cause the session to hang and trigger the `RALPH_TASK_TIMEOUT` kill-and-stall mechanism. Run only commands that terminate on their own — the `Verify:` command and short, bounded shell commands.
